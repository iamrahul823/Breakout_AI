...
def create_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS email_schedule
                 (email_id TEXT, send_time TEXT, status TEXT)''')
    conn.commit()
    conn.close()
...
def get_scheduled_emails():
    conn = sqlite3.connect('email_scheduler.db')
    c = conn.cursor()
    c.execute("SELECT email_id, send_time FROM email_schedule WHERE status='scheduled'")
...
send_bulk_emails_with_schedule(data, throttle_rate=2, send_time="11:08")
